  ...
  (let ((lex (get *lang* 'lexicon)))
    ...
    (multiple-value-bind
     ...
     (mvb-if rootdct (gethash root posdct)
             (error (format nil "Attempting to re-register root ~a with part of speech ~a in language ~a." root pos *lang*))
             (setf (gethash root posdct) args)))))
(defun register-many-morphs (format args roots)
  (let ((keys (uiop:ensure-list format)))
    (unless (member :display keys)
...
(defun add-rule (pos root type layer rule)
  (let-if lex (get *lang* 'lexicon)
          (mvb-if posdct (gethash pos lex)
                  (mvb-if mrph (gethash root posdct)
                          (push (list layer rule)
                                (getf (gethash root posdct) type))
                          (error (format nil "Root ~a does not exist with part of speech ~a in language ~a." root pos *lang*)))
                  (error (format nil "Language ~a has no part of speech ~a." *lang* pos)))
          (error (format nil "Language ~a has no lexicon." *lang*))))
